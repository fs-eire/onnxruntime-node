...
//
{
  (async () => {
    ...
    // pass `Feed` interface
    const result00 = await mySession.run({names: ['data0', 'data1'], values: [input0, input1]});

    // pass values only. `names` can be omitted, if it's the same as model's input name list, and you are very sure
    // about it.
    const result01 = await mySession.run({values: [input0, input1]});
    ...
    const result11 = await mySession.run(inputFeed, {names: ['output0', 'output1']});
